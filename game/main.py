#!/usr/bin/env python3
"""ARE YOU READY? - Launcher v1.1 (CustomTkinter)"""
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("ARE YOU READY?"); self.geometry("540x680"); self.resizable(False, False)
        self.configure(fg_color=("#1a1a2e", "#1a1a2e"))
        self._build()
    def _build(self):
        ctk.CTkLabel(self, text="ARE YOU READY?", font=ctk.CTkFont(family="Impact", size=48), text_color="#e94560").pack(pady=(35,0))
        ctk.CTkLabel(self, text="TACTICAL SPECIAL FORCES", font=ctk.CTkFont(size=14), text_color="#555570").pack(pady=(0,5))
        ctk.CTkLabel(self, text="v1.1", font=ctk.CTkFont(size=11), text_color="#444460").pack(pady=(0,20))
        ctk.CTkFrame(self, height=2, fg_color="#333355").pack(fill="x", padx=40, pady=5)
        ctk.CTkLabel(self, text="SELECT MAP", font=ctk.CTkFont(size=13, weight="bold"), text_color="#888899").pack(pady=(15,5))
        ...
        self.info_lbl = ctk.CTkLabel(self, text="", font=ctk.CTkFont(size=11), text_color="#555570"); self.info_lbl.pack(pady=(8,0))
        ...
        ctk.CTkFrame(self, height=2, fg_color="#333355").pack(fill="x", padx=40, pady=15)
        ctk.CTkLabel(self, text="WASD / \u0426\u0424\u042b\u0412 - Move", font=ctk.CTkFont(size=10), text_color="#444460").pack()
        ctk.CTkLabel(self, text="E / \u0423 - Interact  |  LMB - Shoot  |  ESC - Pause", font=ctk.CTkFont(size=10), text_color="#444460").pack(pady=(2,10))
    def _find_maps(self):
        md = os.path.join(GAME_DIR, "maps"); os.makedirs(md, exist_ok=True)
        dp = os.path.join(md, "default.json")
